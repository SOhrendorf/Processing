...
void setup() {
  ...
  //sonstige Vorbereitungen (z.B. Highscore & Textoptionen)
  frameRate(60);
  ...
  for (int i = 0; i < pAnzahlAliens; i++) {
    ...
    float posX = random(i*sectorMaxXPixel, (i*sectorMaxXPixel)+50);
    float posY = random(70, height-70);
    int dx = parseInt(random(-5, 5));
    int dy = parseInt(random(-5, 5));
    dx = variableNotNull(dx); 
    dy = variableNotNull(dy);
    if(i==0){
      //erzeugen eines EvilAliens zum Begin
      float speedMultipler = random(0.01, 0.05);
      EvilAlien alien= new EvilAlien(dx,dy,posX,posY,speedMultipler);
      alienList[i]=alien;
      alien.setup();
    }else{
...
void draw() {
  ...
  //alle 60 Sekunden ein neues Alien erschaffen
  if (frameCount%600 == 0){
  ...
  text("highscore: "+ highscore, width-150, 70 );
